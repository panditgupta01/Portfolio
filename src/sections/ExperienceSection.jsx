          ...
          const linkEntries =
            item.links?.length > 0
              ...
              : item.linkHref && item.linkLabel
                ? [{ label: item.linkLabel, href: item.linkHref }]
                : []
                ...
                {linkEntries.map((link) => {
                  ...
                  return (
                    <a
                      key={link.href}
                      href={link.href}
                      {...(isExternal ? { target: '_blank', rel: 'noopener noreferrer' } : { target: '_blank' })}
                      className="text-sm font-medium text-cyan-300 underline-offset-4 transition hover:text-cyan-200 hover:underline"
                    >
                      {link.label}
                    </a>
                  )
